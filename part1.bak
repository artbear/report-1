...
@para{Если использование этих конструкций вызывает у вас сложности, обратитесь к официально документации}
...
@para{Согласно методологии TDD сперва пишется тест, затем код. В этой статье применен другой подход – тест является информацией как использовать тот или иной метод. Функция контроля корректности кода не используется в полной мере.}}
          @item{
          ...
          @;@item{@para{Статья написана с помощью системы генерации текста Scribble (входит в состав DrRacket, скачать можно здесь - @url{http://racket-lang.org/download/})}}
                                                                                                                                                                              ]
@section{Введение}
...
@para{
      ...
      В этой статье я буду описывать процесс построения парсера по алгоритму нисходящего разбора, как это описано в статье 
      ...
      После того как будут описаны все необходимые примитивы, будет разработана система генерации парсеров на основании простого текстового описания}
...
@1C{
      Функция РазобраноУспешно(Значение, Остаток)
          Возврат новый Структура("Тип, Значение, Остаток",1,Значение,Остаток);
      КонецФункции
      Функция РазобраноНеудачно(Остаток)
          Возврат новый Структура("Тип, Остаток",0,Остаток);
      КонецФункции
      }
@para{Тогда определение парсера одного символа можно описать так}
@1C{
...
@1C{
      ...
      Процедура Тест_АтомарныйУспешно() экспорт
          ...
          юТест.ПроверитьРавенство(результат.Тип, образец.Тип, "Тип - успешно разобрали");
          юТест.ПроверитьРавенство(результат.Значение, образец.Значение, "Значение - успешно разобрали");
          юТест.ПроверитьРавенство(результат.Остаток, образец.Остаток, "Остаток - успешно разобрали");
      КонецПроцедуры
      
      Процедура Тест_АтомарныйНеудачно() экспорт
          образец = новый структура("Тип,Остаток",0,"мама");
          результат = ПарсерОдногоСимвола("М","мама");
          юТест.ПроверитьРавенство(результат.Тип, образец.Тип, "Тип - успешно неудачно разобрали");
          юТест.ПроверитьРавенство(результат.Остаток, образец.Остаток, "Остаток - успешно неудачно разобрали");
      КонецПроцедуры}
...
@para{Последовательность парсеров - это комбинация 2 парсеров, работающая следующим образом -}
@itemize[
         @item{
...
@para{Для начала - определимся с режимами, в которых будет существовать наш парсер. Их 2 - режим компиляции и режим выполнения.
...
@para{В нашей реализации парсер будет представляться структурой с обязательным полем "Тип" и списком аргументов которые будут применяться для каждого типа парсера
      Реализация функции ПостроитьПарсер приведена ниже
      }
@1C{
...
@para{А функция применения парсера тогда выглядит так}
@1C{    
...
@para{Самая сложная для понимания часть закончена. Дальше будет проще.}
