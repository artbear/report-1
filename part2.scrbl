...
@BNF[(list @nonterm{digits} @kleeneplus[@nonterm{@optional[@BNF-seq[@litchar{0-9}]]}])
     ...
     (list @nonterm{T} @BNF-alt[
                                @BNF-seq[@nonterm{T} @BNF-alt[@litchar{*} @litchar{/}] @nonterm{F}]
                                 @nonterm{F}]) 
     (list @nonterm{S} @BNF-alt[
                                @BNF-seq[@nonterm{S} @BNF-alt[@litchar{+} @litchar{-}] @nonterm{T}]
                                 @nonterm{T}])]
...
Функция Парсер_РазобратьЧисло()
	...
	T = _choose(_seq(T,_choose(_match("*"),_match("/")),F),F); // ~T ("*" | "/") F | F~
	S = _choose(_seq(S,_choose(_match("+"),_match("-")),T),T); // ~S ("+" | "-") T | T~
	Возврат S;
КонецФункции}
