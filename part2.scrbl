...
@margin-note{ Как вы видите - увиличилось количество параметров, усложнилась немного логика выбора парсера и после взгляда на функцию @1C{ПрименитьОперацию1()} возникает  дежавю - 
уж не франчайзи ли этот код пишут.Дабы развеять опасения - нет, не франчайзи. Сертификтов не получал, писать так же красиво, как некоторые франчази "Входящие в TOP-5" не умею, но иногда, как вы видите - получается.}
...
@para{В замечательной книге @link["http://yandex.ru/yandsearch?text=%D0%BF%D1%80%D0%BE%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B8%D1%81%D1%82%20%D0%BF%D1%80%D0%B0%D0%B3%D0%BC%D0%B0%D1%82%D0%B8%D0%BA"]{Эндрю Ханта "Программист-прагматик"} 
есть глава посвященная генерации текстов программ. Конкретно сейчас нам интересны "активные генераторы", то есть такие которые на основании модели кода генерируют текст. }
@para{Самые продвинутые читатели конечно же вспомнили сейчас о таких программах как yacc, bison, ANTLR. О плюсах которые они дают, ну и конечно же о минусах. И конечно же жогадались что мы сейчас будем делать. Что бы не обманывать их ожиданий скажу - да, мы сейчас напишем генератор лексических анализаторов. Только вот похож он будет больше на 
@link["http://waxeye.org/"]{waxeye}
либо 
@link["https://github.com/pegjs/pegjs"]{PEG.js}
}
      @subsection{Из чего состоит генератор?}
@para{Генератор текста программы состоит из описания модели, парсера языка модели и блока формирования/вывода кода.}
@para{Таким образом в ближайшее время нам предстоит разработать либо взять готовый язык описания грамматики, структуру внутреннего представления прочитаной и разобранной грамматики и сделать процедуру генерации текста программы по внутреннему представлению.}
@subsection{Что бы понять рекурсию - надо понять рекурсию}
@para{Итак, мы начнем с разработки грамматики определяющей грамматику которую будет уметь обрабатывать генератор парсеров.}
@para{Звучит как "масло-маслянное" на первый взгляд. Но это не так. Попробуйте прочитать первое предложение еще раз вдумчиво и 
поймете что это не так. Кстати, если вы вспомнили после второго прочтения слово "метапрограммирование" - забудьте.}
@margin-note{К этой задаче и этому языку метапрограммирование - не имеет отношения. Я, вообще, слабо представляю метапрограммирование на 1С. Может быть в 1С версии языка 100500 и появятся макросы, 
...
@para{Грамматику для нашего файла определения грамматики возмем  @link["https://github.com/orlandohill/waxeye/blob/master/grammars/waxeye.waxeye"]{отсюда} и модифицируем для наших целей.
   Изменим немного функции _seq и _choose, и получиться примерно следующее:}
...
Функция СоздатьПарсер(СтрокаОпределенияГрамматики)
	...
	ДобавитьПравило(СтруктураПарсера,"Grammar",_seq("Ws","Definition_star"));
	ДобавитьПравило(СтруктураПарсера,"Definition_star",_star("Definition"));
	ДобавитьПравило(СтруктураПарсера,"Definition",_seq("Identifier","Arrow","Alternation","Ws"));
	ДобавитьПравило(СтруктураПарсера,"Alternation",_seq("Sequence","AlternationPart_star"));
	ДобавитьПравило(СтруктураПарсера,"AlternationPart_star",_star("AlternationPart"));
	ДобавитьПравило(СтруктураПарсера,"AlternationPart",_seq("Alt","Sequence"));
	ДобавитьПравило(СтруктураПарсера,"Sequence",_plus("Unit"));
	ДобавитьПравило(СтруктураПарсера,"Unit",_seq("Prefix_star","UnitPart"));
	ДобавитьПравило(СтруктураПарсера,"Prefix_star",_star("Prefix"));
	...
	ДобавитьПравило(СтруктураПарсера,"PrefixPart",_match_or_list(СтруктураПарсера,"PrefixChar","?*+&!"));
	ДобавитьПравило(СтруктураПарсера,"Identifier",_seq("IdFirst","IdRest_star","Ws"));
	ДобавитьПравило(СтруктураПарсера,"IdFirst",_choose("range_az","range__AZ","range_ая","range__АЯ","underscore"));
	ДобавитьПравило(СтруктураПарсера,"range_09",_range("0","9"));
	ДобавитьПравило(СтруктураПарсера,"range_az",_range("a","z"));
	ДобавитьПравило(СтруктураПарсера,"range__AZ",_range("A","Z"));
	ДобавитьПравило(СтруктураПарсера,"range_ая",_range("а","я"));
	ДобавитьПравило(СтруктураПарсера,"range__АЯ",_range("А","Я"));
	ДобавитьПравило(СтруктураПарсера,"underscore",_match("_"));
	ДобавитьПравило(СтруктураПарсера,"IdRest_star",_star("IdRest"));
	ДобавитьПравило(СтруктураПарсера,"IdRest",_choose("range_az","range__AZ","range_ая","range__АЯ","underscore","range_09"));
	ДобавитьПравило(СтруктураПарсера,"Literal",_seq("OpenQuote","Literal_star","CloseQuote","Ws"));
	ДобавитьПравило(СтруктураПарсера,"OpenQuote",_match("'"));
	ДобавитьПравило(СтруктураПарсера,"CloseQuote",_match("'"));
	ДобавитьПравило(СтруктураПарсера,"Literal_star",_star("LiteralPart"));
	...
	ДобавитьПравило(СтруктураПарсера,"SpecialChar",_match_or_list(СтруктураПарсера,"SpecialCharList","nrt'""\"));
	...
	ДобавитьПравило(СтруктураПарсера,"Open_1",_match("["));
	ДобавитьПравило(СтруктураПарсера,"Close_1",_match("]"));
	ДобавитьПравило(СтруктураПарсера,"CharClass",_seq("Open_1", "Ranges" , "Close_1","Ws"));
	ДобавитьПравило(СтруктураПарсера,"Ranges",_star("RangeElement"));
	ДобавитьПравило(СтруктураПарсера,"NotClose_1", _not("Close_1"));
	ДобавитьПравило(СтруктураПарсера,"RangeElement",_seq("NotClose_1","Range"));
	...
	ДобавитьПравило(СтруктураПарсера,"Char0",_seq("Slash","SpecialChar_1"));
	ДобавитьПравило(СтруктураПарсера,"SpecialChar_1",_match_or_list(СтруктураПарсера,"SpecialCharList_1","nrt-]\"));
	ДобавитьПравило(СтруктураПарсера,"Char1",_seq("NotSlash", "NotClose_1","NotEOL","Any"));
	...
	ДобавитьПравило(СтруктураПарсера,"EndOfLine",_match_or_list(СтруктураПарсера,"EOL",Символы.ПС+Символы.ВК));
	ДобавитьПравило(СтруктураПарсера,"WsPart",_choose(ДобавитьПравило(СтруктураПарсера,"SpaceTab",_match_or_list(СтруктураПарсера,"SpaceTabValue"," "+Символы.Таб)),"EndOfLine","SComment"));
...
@para{Что бы не загромождать текст кодом, тест здесь не публикую, но в файле arith5.epf он есть. Как видите - назначение этой функции по переданной грамматике разобрать строку и получить внутренее представление для дальнейшей обработки. Фактически самый сложный этап мы сделали. Осталось всего ничего - написать интерпретатор для обхода получившейся структуры.
      Чем мы и займемся.}
